...
def load_file(filepath, res=0.05):
    ...
    if os.path.exists(filepath):
        ...
        voxel_size = stl_file.bounds[1][1]/voxel_grid.shape[1] #mm
        ...
        if command == ' ' or command == '':
            ...
            for i in range(len(cpos)):
                ...
                if cpos[i] == 'front':
                    ...
                    img_plot.camera.azimuth = 45
                    img_plot.render()
                img_plot.screenshot(f'temp/{cpos[i]}.png', transparent_background=True)
            ...
            try:
                if sl is not None:
                    mdfile.new_paragraph(f'**Number of Standard Lengths:** *{math.ceil(mydict["Pipe Length"]/sl):.2f}*')
            except NameError:
            ...
            l_images = []
            for i in range(len(cpos)):
                l_images.append(f'./temp/{cpos[i]}.png')
            mdfile.new_paragraph()
            ...
            if not os.path.exists('reports'):
                os.makedirs('reports')
            try:
                doc = pypandoc.convert_file('report.md', 'pdf', outputfile='reports/report.pdf', extra_args=['-V', 'geometry:margin=1in'])
            except RuntimeError as e:
                print(e)
                print("Error converting to pdf. Install MikTeX for direct PDF conversion. Word file generated instead.")
                doc = pypandoc.convert_file('report.md', 'docx', outputfile='reports/report.docx')
            os.remove('report.md')
